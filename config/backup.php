...
/*
 * Backup notifications are opt-in: they require working mail (SMTP). Until that's
 * configured, leave BACKUP_NOTIFICATIONS_ENABLED off so a missing mailer never
 * makes `backup:run` exit with an error after a successful backup.
 */
...
return [
    ...
    'backup' => [
        /*
         * The name of this application — also the folder/prefix backups are
         * stored under (e.g. on S3: {prefix}/YYYY-MM-DD-….zip). Slugged so it's
         * safe as a path. Tenant-ready: set BACKUP_PREFIX per tenant later.
         */
        ...
        'source' => [
            'files' => [
                 ...
                 */
                /*
                 * Only back up irreplaceable data: user uploads (storage/app) and
                 * the .env (secrets, not in git). The codebase itself is in git /
                 * your deploy, so there's no point zipping it on every run.
                 */
                'include' => [
                    storage_path('app'),
                    base_path('.env'),
                ...
                /*
                 * Directories used by the backup process are auto-excluded, but be
                 * explicit so previous backups / temp files never nest inside new ones.
                 */
                'exclude' => [
                    storage_path('app/' . (env('BACKUP_PREFIX')
                        ?: (\Illuminate\Support\Str::slug(env('APP_NAME', 'backup')) ?: 'backup'))),
                ...
                /*
                 * Store files relative to the project root, so the zip contains
                 * storage/app/… and .env at the top level instead of home/user/….
                 */
        ...
        'destination' => [
            ...
            /*
             * The disk names on which the backups will be stored. Env-driven so
             * you can add off-site storage without code changes, e.g.
             * BACKUP_DISKS=local,s3 (s3 disk works with AWS/R2/B2/Spaces).
             */
    ...
    'notifications' => [
        ...
        'mail' => [
            // Always a valid address (spatie validates it even when mail channel is off).
            'to' => env('BACKUP_NOTIFICATION_EMAIL') ?: (env('SCHOOL_EMAIL') ?: 'admin@example.com'),
